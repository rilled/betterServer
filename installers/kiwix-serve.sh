#!/bin/bash
# Modified & simplified, credit to the creator:
...
# TODO make check if file is there
...
# Check if save directory exists already
if [[ -d $SAVEDIR ]]; then
    mkdir -p "$SAVEDIR"
    printf "%b\n" "${Green}Created ${SAVEDIR} ${Color_Off}"
else
    printf "%b\n" "${Green}Specified directory already exists!${Color_Off}"
fi
...
# enable services on boot & check status
systemctl enable --now kiwix-serve.service
systemctl status kiwix-serve.service
